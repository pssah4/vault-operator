...
related:
  ...
  - PLAN-005-feature-0316-memory-migration-vault-rrf.md
  - _devprocess/requirements/features/FEATURE-0317-dynamic-context-composition.md
  - _devprocess/analysis/SPIKE-001-cross-db-performance.md
---

# PLAN-006 -- Memory v2 Phase 3 Dynamic Context Composition

## Kontext

Phase 2 (PLAN-005) lieferte die Migration + Hybrid-Search-Bausteine. Phase 3
...
- **ADR-080 / SPIKE-001**: Cross-DB-Walks via JS-Layer-BFS, NICHT ATTACH.
  LocalKnowledgeAdapter führt SELECT auf separaten sql.js-Instanzen aus
  und joint die Resultate in JavaScript.
- **ADR-078**: Resolution geht durch SourceAdapter-Registry. Unbekannte
  URIs bleiben Reference-Tokens, kein Crash.
- **ADR-082 (Topic-Inference)**: Centroid-Cosine über `known_topics`
  (Phase-1-Schema), kein LLM-Call beim Conversation-Start. Sub-50ms.
- **ADR-081 (MCP Routing)**: Standalone-Service-Setup nutzt
  McpKnowledgeAdapter (RPC zu Plugin-MCP). LAN-RTT 20-50ms, akzeptabel.
- **ADR-077 / FEATURE-0315**: `known_topics.centroid_embedding`-Spalte
...
PLAN-006 erstellt. Status: Active. Trigger: User-Anweisung "weiter im
