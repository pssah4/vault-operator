...
## Initiative: Memory v2 + UCM Foundation (Phase 5 abgeschlossen, 2026-04-28)
...
**Naechster Schritt:** Phase 6 (FEATURE-0320) -- History Search: history_chunks-Tabelle befuellen, HistoryIndexer (incremental + abortable backfill), `search_history`-Tool, UI-Sidebar-Search. Phase 5 hat den Re-Extraction-Throttle, Aging-Scheduler im Onload (Two-Tier-Halflife mit Pattern-Threshold), Pinned-Conversation Lower Threshold, History-Cascade-Delete, Cold-Start-Hint, FactIntegrator-Dedup-Pre-Check und Save-to-Memory ueberall live (Star + Menu + Command). Auto-Suggestion + thread://-Host-Wiring deferred zur UCM-Initiative. Combined Note-Index-Pass (4.1) bleibt deferred.
