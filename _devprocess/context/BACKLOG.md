...
| FEAT-29-05 | Feature | Skill-Creator-Builtin-Skill | Done | Building | EPIC-29, FEAT-29-02, FEAT-29-06, ADR-126 | USER |  | sebastian-claude-opus-4-7 | 2026-05-21 | P1 Welle 3-4. /coding 2026-05-21. Bundle skill-creator builtin (SKILL.md + init_skill.js + quick_validate.js + 2 references). manage_skill-Tool komplett entfernt (Wiring, ToolName-Union, toolMetadata, AgentSidebarView, builtinModes, capabilities.ts/objective.ts prompts). SkillFrontmatterValidator als discovery-gate (16 Tests). TaskRouter COMPLEX_SKILL_CREATION_RE eskaliert auf Flagship (2 Tests). SandboxBridge erweitert um vault.mkdir + hidden-path support (vault-operator-Folder zugreifbar). EditFileTool adapter-fallback fuer hidden paths. UI-Polish: user-message-bubble accent-color-mix 20%, customize-tab toolbox icon. Validator-Allowlist erweitert um version/keywords/author/when-to-use/argument-hint/context. VaultDNAScanner toKebabCase fuer plugin-id im name-Frontmatter. Live-Test bestaetigt: weekly-meeting-summary erstellt + valide unter data/skills/. 1918/1939 Tests gruen. Wartet auf /testing + /security-audit. Spec: `_devprocess/requirements/features/FEAT-29-05-skill-creator-builtin.md` |
