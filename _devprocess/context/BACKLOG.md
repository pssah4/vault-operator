...
Last update: 2026-05-21 by coding (FEAT-29-13 Phase 1: Source-Labels-Fix. Drei klar getrennte Labels im SkillsTab: Built-in (bundled/builtin), Agent (skill-creator-Output via neuem `source: agent`-Discriminator, plus Legacy `learned`), User (manuell). `getSourceLabel` + `SOURCE_TOOLTIP` aus SkillsTab nach `userSkillSource.ts` extrahiert -- testbar ohne Modal-Imports. SkillsTab-Header bekommt setTooltip mit Drei-Kategorien-Erklaerung. BuiltinSkillMaterializer-Override-Check erweitert auf `agent` + `learned` (kein Wipe agent-erstellter Skills bei Plugin-Reload). CSS-Klassen ergaenzt (.agent-skill-source-builtin, .agent-skill-source-agent). init_skill schreibt jetzt `source: agent`, skill-creator/SKILL.md Doku aktualisiert. 10/10 userSkillSource-Tests gruen, Build clean. Spec: `_devprocess/requirements/features/FEAT-29-13-skill-source-labels.md`. Phase 2 (Templates-Setup) und Phase 3 (Ingest-Sense-Making) folgen als FEAT-29-14 und FEAT-29-15 in separaten Branches/PRs. Plan: `~/.claude/plans/radiant-snacking-piglet.md`.)
...
| FEAT-29-13 | Feature | Skill-Source-Labels (Built-in, Agent, User) mit Tooltip | Active | Building | EPIC-29, FEAT-29-05, FEAT-29-11 | USER |  | sebastian-claude-opus-4-7 | 2026-05-21 | P2 Welle 4. /coding 2026-05-21: Source-Label `Template` -> `User`, neuer `agent`-Wert via skill-creator `init_skill`, drei Buckets im SkillsTab (Built-in/Agent/User), Tooltip am Header. `getSourceLabel` + `SOURCE_TOOLTIP` extrahiert nach `userSkillSource.ts` (testbar). BuiltinSkillMaterializer-Override-Check schliesst `agent` + `learned` ein. CSS-Klassen fuer alle Source-Werte. 10/10 userSkillSource-Tests gruen. Spec: `_devprocess/requirements/features/FEAT-29-13-skill-source-labels.md`. Naechste Phasen separat: FEAT-29-14 (Templates-Setup) + FEAT-29-15 (Ingest-Sense-Making). |
