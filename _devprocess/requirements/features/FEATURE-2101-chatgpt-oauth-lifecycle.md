...
PKCE-OAuth-Flow gegen `auth.openai.com` mit lokalem Loopback-Callback-Server, Token-Persistenz ueber SafeStorageService und automatischem Refresh. Das Feature kapselt die komplette Token-Lebensdauer in einem dedizierten Service, sodass die spaeteren API-Calls (FEATURE-021-002) nur noch ein gueltiges `access_token` plus `chatgpt-account-id` aus dem Service ziehen muessen, ohne sich um Auth-State-Maschinen oder Token-Erneuerung zu kuemmern.
