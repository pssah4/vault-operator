...
- **Why ASR**: Multiple features depend on cluster membership (FEATURE-2003, FEATURE-2004). In-memory-only results would require re-computation.
...
- [ ] Integration test: cluster data available to FEATURE-2004
...
**If disproven:** Fall back to simpler presentation: show "potentially related notes" without claiming cluster structure. Use only for retrieval boosting (FEATURE-2004), not user-facing diagnostics.

---

## Dependencies
- **FEATURE-2001** (Confidence Scoring): Confidence-weighted edges produce better clustering input
