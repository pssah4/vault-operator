...
export const en: Translations = {
    ...
    'settings.embeddings.sectionRerankingInfo': 'A second pass that re-orders the top-K search results using a small local model. Embeddings are fast but coarse; the reranker fixes obvious ranking errors when the first hit "looks similar" but is not actually what you asked for. Runs entirely offline.',
    ...
    'settings.webSearch.sectionGeneralInfo': 'The master switch for everything the agent does outside Obsidian. With this off, the agent has no way to fetch a URL or run a search regardless of the rest. With it on, it still respects the auto-approval rules under Permissions.',
