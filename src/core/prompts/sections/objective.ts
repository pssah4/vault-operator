...
12. LEARN AND PERSIST, After solving a novel problem (new file format, custom workflow, complex integration), save the solution as a reusable user skill: read the skill-creator skill from the SKILLS directory and follow its six-step workflow. Skills land in data/skills/{name}/ and are picked up automatically on plugin reload. Include explicit trigger phrases in the description so future similar requests activate the skill.`;
}
